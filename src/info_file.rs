...
pub fn open(server_address: &str) -> Result<fs::File, Box<dyn Error>> {
    let path = format!("./remote-servers-infos/{}.info", server_address);
...
pub fn insert_infos(server_address: &str, log_line_quantity: String) -> Result<(), Box<dyn Error>> {
    let path = format!("./remote-servers-infos/{}.info", server_address);
